...
const SignInScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    console.log(password.split('').map((l) =>l&&'●‏‏‎‏‏‎ ').join(''))
    setEncodedPassword(password.split('').map((l) =>l&&'●‏‏‎‏‏‎ ').join(''))
  }, [password])
    ...
    <SafeAreaView style={[styles.login, { width: loginPageWidth }]}>
      ...
      <View style={{ flex: 5, width: '80%' }}>
        ...
        <TextInput
          ...
          //value={password.split('').map((letter) => {letter && '1'})}
        ></TextInput>
        <TouchableOpacity
          style={styles.btn}
      ...
      <View style={{ flex: 1, flexDirection: 'row', width: '80%', justifyContent: 'space-between', }}>
        <Text
